...
def listify(v: Any) -> list[Any] | None:
    if v is None:
        return None
...
class EnumFieldListFilter(ChoicesFieldListFilter):
    field: EnumFieldMixin

    def choices(self, changelist: ChangeList) -> Iterator[dict[str, Any]]:  # type: ignore[override]
        ...
        for enum_value in self.field.enum:
            ...
            yield {
                ...
                'query_string': changelist.get_query_string({self.lookup_kwarg: str_value}),
                'display': getattr(enum_value, 'label', None) or force_str(enum_value),
            }

    def queryset(self, request: HttpRequest, queryset: QuerySet) -> QuerySet | None:  # type: ignore[type-arg]
        list_val = listify(self.lookup_val)
        ...
        try:
            ...
            for enum_value in self.field.enum:
                if force_str(enum_value.value) == last_val:
                    param_value: Any = enum_value if django.VERSION < (5,) else [enum_value]
